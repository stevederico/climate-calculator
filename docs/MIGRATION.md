...
Storage is SQLite via system `libsqlite3`, path in `backend/config.json`.
...
`version` in `package.json` is this app (0.17.0). `skateboardVersion` is the template pin (5.6.0). They are not the same number.
